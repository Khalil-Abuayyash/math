...
class VariablesIntroduction(Scene):
    def construct(self):
        ...
        # Display variables with their values in the center
        # Create variable representations
        x_value = Text('x = 5', font_size=40, color=BLUE)
        y_value = Text('y = 10', font_size=40, color=GREEN)
        z_value = Text('z = 15', font_size=40, color=YELLOW)

        # Explanatory texts indicating they are integers
        x_explain = Text('(integer)', font_size=30, color=BLUE)
        y_explain = Text('(integer)', font_size=30, color=GREEN)
        z_explain = Text('(integer)', font_size=30, color=YELLOW)

        # Arrange variables vertically in the center
        variables_group = VGroup(
            VGroup(x_value, x_explain).arrange(RIGHT, buff=0.3),
            VGroup(y_value, y_explain).arrange(RIGHT, buff=0.3),
            VGroup(z_value, z_explain).arrange(RIGHT, buff=0.3)
        ).arrange(DOWN, buff=0.5).move_to(ORIGIN).shift(DOWN * 0.5)

        # **Animate Variables Appearing in Center**
        self.play(FadeIn(variables_group[0]))
        self.wait(0.5)
        self.play(FadeIn(variables_group[1]))
        self.wait(0.5)
        self.play(FadeIn(variables_group[2]))
        self.wait(0.5)
        ...
        ).move_to([0, -4, 0])  # Moved a bit lower for space
        ...
        conclusion_text = Text(
            'Variables store integer values we can use!', 
