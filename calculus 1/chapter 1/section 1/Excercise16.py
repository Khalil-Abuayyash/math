from manim import *

config.pixel_width = 1920   # Set width (e.g., Full HD resolution)
config.pixel_height = 1080  # Set height
config.frame_rate = 30      # Set frame rate (optional)

class Excercise16(Scene):
    def construct(self):
        # Title
        title = Tex(r"Solution to $f(x) = 1 - 2x - x^2$", font_size=50).set_color(BLUE)
        title1 = Tex(r"The Graph of $f(x) = 1 - 2x - x^2$", font_size=50).set_color(BLUE)

        # Natural Domain
        domain_text = Tex(
            r"Natural Domain: $(-\infty, \infty)$ (all real numbers)", 
            font_size=40
        ).set_color(GREEN)
        
        # Explanation
        explanation = Tex(
            r"""
            Explanation: $f(x) = 1 - 2x - x^2$ is a quadratic function.\\
            Quadratic functions are defined for all $x \in \mathbb{R}$.\\
            Therefore, the natural domain is $(-\infty, \infty)$.
            """,
            font_size=40
        ).set_color(WHITE)
        
        # Axes and Graph
        axes = Axes(
            x_range=[-5, 5, 1],  # x-axis range
            y_range=[-10, 10, 1], # y-axis range
            tips=True
        ).add_coordinates()
        
        axes.shift(DOWN* 0.5)
        
        graph = axes.plot(
            lambda x: 1 - 2 * x - x**2, 
            color=RED, 
            x_range=[-5, 5]
        )
        
        # graph_label = axes.get_graph_label(
        #     graph, label= "f(x) = 1 - 2x - x^2", x_val=-2, direction=UR
        # )
        
        # graph_label.shift(UP *  + RIGHT * 3)
        
        # Animations
        self.play(Write(title))
        self.wait(2)
        self.play(title.animate.to_edge(UP))  # Move title up
        self.play(Write(domain_text))
        self.wait(2)
        self.play(domain_text.animate.next_to(title, DOWN, buff=1))
        self.play(Write(explanation))
        self.wait(5)
        self.play(FadeOut(title,domain_text, explanation))
        self.play(Write(title1))
        self.wait(1)
        self.play(title1.animate.to_edge(UP))  # Move title up
        self.play(Create(axes), Create(graph))  # Create axes and graph
        self.wait(9)
        self.play(FadeOut(axes, graph))  # Fade out all elements
